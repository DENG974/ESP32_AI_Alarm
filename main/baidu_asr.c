...
esp_err_t baidu_asr_recognize(int16_t *audio_data, int audio_len, char **result) {
    ...
    *result = NULL;
    ...
    size_t json_size = 512 + base64_len;
    char *json_buffer = malloc(json_size);
    if (!json_buffer) {
        free(base64_data);
        snprintf(g_last_error, sizeof(g_last_error), "JSON缓冲区分配失败");
        return ESP_FAIL;
    }
    
    snprintf(json_buffer, json_size,
            ...
            if (resp_root) {
                ...
                if (err_no && err_no->valueint == 0) {
                    ...
                    if (result_arr && cJSON_GetArraySize(result_arr) > 0) {
                        ...
                        snprintf(g_last_error, sizeof(g_last_error), "无识别结果");
                    }
                } else {
